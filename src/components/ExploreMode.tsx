...
export const ExploreMode: React.FC<ExploreModeProps> = ({
  ...
  return (
    <div className="app-game-card w-full flex-1 min-h-0 flex flex-col md:flex-row gap-2.5 sm:gap-3.5 items-stretch overflow-hidden">
      ...
      <div className="w-full md:w-[360px] lg:w-[400px] bg-white rounded-2xl sm:rounded-3xl p-2.5 sm:p-3 shadow-xs border border-slate-200/80 flex flex-col items-center justify-between shrink-0 overflow-hidden">
        {/* Helper Note for Kids & Live Time Toggle */}
        <div className="w-full flex items-center justify-between gap-1.5 text-xs sm:text-sm font-black text-slate-700 mb-1 px-0.5 shrink-0">
          <span className="flex items-center gap-1.5 text-amber-900 shrink min-w-0">
            <Sparkles className="w-3.5 h-3.5 text-amber-600 shrink-0" />
            <span className="text-[11px] sm:text-xs font-black text-amber-950 whitespace-nowrap">
              {lang === 'en'
          ...
          <button
            id="explore-live-time-btn"
            onClick={() => {
              sounds.playClick();
              onUpdateSettings({ isLiveTime: !settings.isLiveTime });
            }}
            className={`px-2 py-0.5 rounded-lg text-[10px] sm:text-[11px] font-black flex items-center gap-1 transition cursor-pointer border shadow-2xs active:scale-95 shrink-0 ${
              settings.isLiveTime
                ? 'bg-emerald-100 border-emerald-300 text-emerald-950 animate-pulse'
                : 'bg-slate-100 border-slate-300 text-slate-700 hover:bg-slate-200'
            }`}
            title={lang === 'en' ? 'Switch to current live time' : 'السَّاعَةُ الْحَيَّةُ'}
          >
            <Clock className="w-3 h-3 text-emerald-600 shrink-0" />
            <span className="whitespace-nowrap">
              {settings.isLiveTime
                ? (lang === 'en' ? 'Live 🔴' : 'سَاعَةٌ حَيَّةٌ 🔴')
                : (lang === 'en' ? 'Live ⏱️' : 'سَاعَةٌ حَيَّةٌ')}
            </span>
          </button>
        </div>
