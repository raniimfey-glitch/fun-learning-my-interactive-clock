    ...
    <div className="app-game-card w-full flex-1 min-h-0 flex flex-col lg:flex-row gap-2.5 sm:gap-3.5 items-stretch overflow-hidden">
      ...
      <div className="w-full lg:w-[380px] bg-white rounded-2xl sm:rounded-3xl p-2.5 sm:p-3.5 shadow-xs border border-slate-200/80 flex flex-col items-center justify-between shrink-0 overflow-hidden">
        ...
        <div className="w-full flex items-center justify-between text-xs sm:text-sm font-black text-slate-700 mb-1 px-1 shrink-0">
          ...
          {settings.isLiveTime && (
            <span className="bg-emerald-100 text-emerald-900 text-[11px] sm:text-xs px-2.5 py-0.5 rounded-full font-black animate-pulse border border-emerald-300 shrink-0">
              {lang === 'en' ? 'LIVE 🔴' : 'حَيَّةٌ الْآنَ 🔴'}
            </span>
          )}
        </div>
        ...
        <div className="w-full mt-2 pt-2 border-t border-slate-100 grid grid-cols-2 sm:grid-cols-3 gap-1.5 text-xs font-black text-slate-800 shrink-0">
          <button
            onClick={() => {
              sounds.playClick();
              onUpdateSettings({ showMinuteNumbers: !settings.showMinuteNumbers });
            }}
            className={`p-2 rounded-xl border flex items-center justify-center gap-1.5 transition cursor-pointer active:scale-95 shadow-2xs ${
          ...
          >
            ...
            <span className="truncate">{lang === 'en' ? 'Minutes (:05-:55)' : 'أَرْقَامُ الدَّقَائِقِ'}</span>
          </button>
          ...
          >
            ...
            <span className="truncate">{lang === 'en' ? 'Fractions' : 'أَلْوَانُ الْأَرْبَاعِ'}</span>
          </button>
          ...
          <button
            ...
            className={`p-2 rounded-xl border flex items-center justify-center gap-1.5 transition col-span-2 sm:col-span-1 cursor-pointer active:scale-95 shadow-2xs ${
              settings.isLiveTime
          ...
          >
            ...
            <span className="truncate">
              {settings.isLiveTime
                ? (lang === 'en' ? 'Stop Live' : 'إِيقَافُ الْحَيِّ')
      ...
      <div className="w-full lg:flex-1 min-h-0 flex flex-col gap-2.5 shrink-1 overflow-hidden">
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl p-3 sm:p-4 shadow-xs border border-slate-200/80 flex-1 min-h-0 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between gap-2 mb-2 pb-2 border-b border-slate-100 flex-wrap shrink-0">
            <h3 className="font-black text-slate-900 text-sm sm:text-base flex items-center gap-1.5">
              <span>
                {lang === 'en'
                  ? 'Time Presets:'
                  : 'أَوْقَاتٌ نَمُوذَجِيَّةٌ لِلتَّجْرِبَةِ:'}
              </span>
            </h3>
            ...
            <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl text-xs font-black">
              ...
              >
                {lang === 'en' ? 'Fractions' : 'الْأَرْبَاعُ وَالنِّصْفُ'}
              </button>
              <button
              ...
              >
                {lang === 'en' ? "O'clock" : 'تَمَامًا'}
              </button>
            </div>
          </div>

          {/* Preset Buttons Grid */}
          <div className="flex-1 min-h-0 overflow-y-auto app-scrollable-card">
            {selectedCategory === 'fractions' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {fractionPresets.map((item, idx) => (
                  <button
                    ...
                    className={`p-2.5 rounded-xl bg-amber-50/80 hover:bg-amber-100 border border-amber-200 transition flex flex-col gap-0.5 group active:scale-95 cursor-pointer shadow-2xs ${
                      lang === 'en' ? 'text-left' : 'text-right'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-black text-slate-900 text-sm group-hover:text-amber-900">
                        {item.label}
                      </span>
                      <Volume2 className="w-4 h-4 text-amber-600 opacity-70 group-hover:opacity-100" />
                    </div>
                    <span className="text-xs text-slate-700 font-bold">{item.desc}</span>
                  </button>
                ))}
              </div>
            )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {dailyPresets.map((item, idx) => (
                  <button
                    key={idx}
                    onClick={() => applyPreset(item.h, item.m)}
                    className={`p-2.5 rounded-xl bg-sky-50/80 hover:bg-sky-100 border border-sky-200 transition flex flex-col gap-0.5 group active:scale-95 cursor-pointer shadow-2xs ${
                      lang === 'en' ? 'text-left' : 'text-right'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-black text-slate-900 text-sm group-hover:text-sky-900">
                        {item.label}
                      </span>
                      <Volume2 className="w-4 h-4 text-sky-600 opacity-70 group-hover:opacity-100" />
                    </div>
                    <span className="text-xs text-slate-700 font-bold">{item.desc}</span>
                  </button>
                ))}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                {hourPresets.map((item, idx) => (
                  <button
                    key={idx}
                    onClick={() => applyPreset(item.h, item.m)}
                    className="py-2.5 px-3 rounded-xl bg-slate-50 hover:bg-amber-50 hover:border-amber-300 border border-slate-200 text-center font-black text-xs sm:text-sm text-slate-900 hover:text-amber-900 transition active:scale-95 cursor-pointer flex items-center justify-between shadow-2xs"
                  >
